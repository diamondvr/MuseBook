...
<head>
    ...
    <title> <?php echo $_SESSION['userRealName']." | Booking History" ?> </title>
    <link rel="stylesheet" type="text/css" href="css/bookHistory-style.css">
...
<body onload="zoom()">
    <?php
        if(!isset($_SESSION['userName'])) {
            ...
            echo '<h1> Booking History </h1><hr>';
            $var_user = $_SESSION['userName'];
            $book_data = mysqli_query($con, " SELECT * FROM booking_detail WHERE user_customer = '$var_user' ");
            if($book_data->num_rows === 0) {
                echo "<h2> You havn't booked any artist yet. </h2>";
            }else{
                    ...
                    if($row['verified'] == 0) {
                        echo ' 
                            <a href="book-detail.php?bookid='.$row['booking_id'].'" class="detail"> 
                                ...
                                <li class="coln">'.$row['artist_name'].'</li>
